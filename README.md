...
- **翻譯術語表**：先讓 AI 從卡片抽出專有名詞，再逐一議定譯名，之後每次翻譯都只把該段文字用得到的詞釘進提示詞裡——200 個詞的卡片也不會撐出一個巨大的 prompt。譯名可鎖定、可標記「保留原文」、可設別名。每個詞旁邊會標出它是哪一條世界書的關鍵字，展開還能看到那條裡實際用到它的那一句——判斷 `parasite` 該譯成寄生蟲還是寄生體，靠的就是這句話，而不是先把整張卡讀完。術語表連同風格註記存在卡片的 `data.extensions.cceditor_plus` 裡，跟著卡片走，也能單獨匯入／匯出 JSON。列表會把還沒決定、以及被檢查點名的詞排到最上面，同樣待處理時看誰出現得多。
...
- **翻譯報告**：每段是成功、被擋、還是被限流一目了然，並附上六種確定性檢查：整段原封退回、`{{char}}` / `{{user}}` 巨集數量對不上、換行結構被改動、繁體中文譯文裡混進簡體字、譯文裡出現 U+FFFD（模型把字打壞了），模型自己寫進正文的譯註。第一種是其餘五種都看不見的：模型不想翻就把原文抄回來，巨集數量一樣、換行一樣、沒有簡體字也沒有壞字元，五項全過——一張卡曾經二十四段裡有十段是這樣退回來的，報告卻說全部成功，連重試都不會回頭找它們。只報告，不自動修——為了滿足啟發式規則去改寫完成的譯文，正是譯文出現破碎句子的原因。
...
- **逐段還原**：每個欄位、開場白與世界書條目各自記著翻譯前的內容，按鈕就在該段的翻譯鍵旁邊。翻譯是原地覆蓋，而模型婉拒或把原文抄回來的那一段看起來就像譯文——沒有這個，被蓋掉的條目就只剩原始檔救得回來。二十段裡壞掉的是第十七段，要救的也只有那一段。按鈕是來回切換不是單向還原，按錯再按一次就回去；翻完又手改過的內容也不會因為還原而消失，它會變成再按一次會回到的東西。還原點跟著草稿走，關掉分頁再打開仍在。
